...
def download_with_proxy(url, proxy_address, timeout=5):
    max_retries = 0  # 最大重试次数
    retries = 1
    while retries > max_retries:
        try:
            proxy_address = my_proxy.get_healthiest_proxy()
            resp = requests.get(url, proxies={'http': proxy_address, 'https': proxy_address}, timeout=timeout)
...
my_proxy = myproxy.Config(proxies, 2, 7)
image_url = "https://www.baidu.com/img/flexible/logo/pc/result.png"
response = download_with_proxy(image_url, my_proxy, timeout=10)
